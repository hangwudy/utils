...
if __name__ == "__main__":
    """
    annotation_file_path = "/home/wu/CyMePro/Object_Detection/annotations/AttitudeNet/car_door_attitude.json"
    output_file_path = "/home/wu/CyMePro/Dataset/annotations/codebook/Codebook.json"
    dict_annotation = dict_generation(annotation_file_path)
    print(dict_annotation)
    write_dict_to_file(annotation_file_path, output_file_path)
    test_dict = read_dict_file(output_file_path)
    print(test_dict["2"]["210"])
    """
    json_file = "Codebook.json"
    dict_length = read_dict_file(json_file)

    """
    for i in [27, 30, 18, 28, 23]:
        for j in np.linspace(0,30, 7, dtype=np.int16):
            print("({}, {}):".format(i,j), dict_length[str(i)][str(int(j))])
        for j in np.linspace(330,355, 6, dtype=np.int16):
            print("({}, {}):".format(i,j), dict_length[str(i)][str(int(j))])
    """

    for i in range(0,45):
        print(len(dict_length[str(i)]))
